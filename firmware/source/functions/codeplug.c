...
bool codeplugZoneGetDataForNumber(int zoneNum, struct_codeplugZone_t *returnBuf) {
	if (zoneNum == (codeplugZonesGetCount() - 1))  //special case: return a special Zone called 'All Channels'
	{
		...
		if (foundIndex != -1) {
			...
			EEPROM_Read(CODEPLUG_ADDR_EX_ZONE_LIST + (foundIndex * (16 + (sizeof(uint16_t) * codeplugChannelsPerZone))), (uint8_t*)returnBuf, (
					(codeplugChannelsPerZone == 16) ? CODEPLUG_ZONE_DATA_ORIGINAL_STRUCT_SIZE : CODEPLUG_ZONE_DATA_OPENGD77_STRUCT_SIZE));
			...
			for (int i = 0; i < codeplugChannelsPerZone; i++) {
				...
				if ((returnBuf->channels[i] == 0) || (i == (codeplugChannelsPerZone - 1))) {
					returnBuf->NOT_IN_CODEPLUGDATA_highestIndex = returnBuf->NOT_IN_CODEPLUGDATA_numChannelsInZone = (i + (
							(returnBuf->channels[i] == 0) ? 0 : 1));
					return true;
				}
			}
		}
...
bool codeplugZoneAddChannelToZoneAndSave(int channelIndex, struct_codeplugZone_t *zoneBuf) {
	if ((zoneBuf->NOT_IN_CODEPLUGDATA_numChannelsInZone <= codeplugChannelsPerZone) && (zoneBuf->NOT_IN_CODEPLUGDATA_indexNumber != -1)) {
		...
		// IMPORTANT. Write size is different from the size of the data, because it the zone struct contains properties not in the codeplug data
		return EEPROM_Write(CODEPLUG_ADDR_EX_ZONE_LIST + (zoneBuf->NOT_IN_CODEPLUGDATA_indexNumber * (16 + (sizeof(uint16_t) * codeplugChannelsPerZone))), (uint8_t*)zoneBuf, (
				(codeplugChannelsPerZone == 16) ? CODEPLUG_ZONE_DATA_ORIGINAL_STRUCT_SIZE : CODEPLUG_ZONE_DATA_OPENGD77_STRUCT_SIZE));
	} else {
		return false;
	}
...
static bool codeplugAllChannelsReadHeaderBank(int channelBank, uint8_t *bitArray) {
	...
	return SPI_Flash_read(CODEPLUG_ADDR_CHANNEL_HEADER_FLASH + ((channelBank - 1) * (CODEPLUG_CHANNELS_PER_BANK * CODEPLUG_CHANNEL_DATA_STRUCT_SIZE + 16)), bitArray, 16);
}
...
void codeplugAllChannelsIndexSetUsed(int index) {
	if ((index >= CODEPLUG_CHANNELS_MIN) && (index <= CODEPLUG_CHANNELS_MAX)) {
		...
		if (channelBank == 0) {
			...
			SPI_Flash_write((CODEPLUG_ADDR_CHANNEL_HEADER_FLASH + ((channelBank - 1) * (CODEPLUG_CHANNELS_PER_BANK * CODEPLUG_CHANNEL_DATA_STRUCT_SIZE + 16))) + byteno, &codeplugAllChannelsCache[cacheOffset], 1);
		}
...
static bool codeplugContactGetReserve1ByteForIndex(int index, struct_codeplugContact_t *contact) {
	if (((codeplugContactsCache.numTGContacts > 0) || (codeplugContactsCache.numPCContacts > 0) || (codeplugContactsCache.numALLContacts > 0)) && (index >= CODEPLUG_CONTACTS_MIN) && (index <= CODEPLUG_CONTACTS_MAX)) {
		index--;
...
static bool quickkeyIsEmpty(char key, uint16_t functionId) {
	...
	if ((data == 0x8000) || (((data & 0x8000) == 0) && ((data < CODEPLUG_CONTACTS_MIN) || (data > CODEPLUG_CONTACTS_MAX)))) {
		return true;
	}
