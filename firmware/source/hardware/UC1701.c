	...
	if (startAngle > endAngle) {
		...
		for (int x = xmin; x <= xmax; x++) {
			...
			for (int y = ymin; y <= ymax; y++) {
				...
				if ((x2 + y2 < or2 && x2 + y2 >= ir2) && ((y > 0 && startAngle < 180 && x <= y * sslope) || (y < 0 && startAngle > 180 && x >= y * sslope) || (y < 0 && startAngle <= 180) || (y == 0 && startAngle <= 180 && x < 0) || (y == 0 && startAngle == 0 && x > 0)) && ((y > 0 && endAngle < 180 && x >= y * eslope) || (y < 0 && endAngle > 180 && x <= y * eslope) || (y > 0 && endAngle >= 180) || (y == 0 && endAngle >= 180 && x < 0) || (y == 0 && startAngle == 0 && x > 0))) {
...
void ucDrawChoice(ucChoice_t choice, bool clearRegion) {
	...
	struct {
		char *lText;
		char *rText;
	} choices[] = { { "OK", NULL },  // UC1701_CHOICE_OK
	...
	const int ucTriangleArrows[2][6] = { {  // Down
			(DISPLAY_SIZE_X / 2) - 12, (TEXT_Y + (FONT_SIZE_3_HEIGHT / 2) - 1), (DISPLAY_SIZE_X / 2) - 4,
			(TEXT_Y + (FONT_SIZE_3_HEIGHT / 2) - (FONT_SIZE_3_HEIGHT / 4) - 1), (DISPLAY_SIZE_X / 2) - 4,
			(TEXT_Y + (FONT_SIZE_3_HEIGHT / 2) + (FONT_SIZE_3_HEIGHT / 4) - 1) },  // Up
		{
	...
	if (choice == CHOICES_OKARROWS) {
		ucFillTriangle(ucTriangleArrows[0][0], ucTriangleArrows[0][1], ucTriangleArrows[0][2], ucTriangleArrows[0][3], ucTriangleArrows[0][4], ucTriangleArrows[0][5], true);
		ucFillTriangle(ucTriangleArrows[1][0], ucTriangleArrows[1][1], ucTriangleArrows[1][2], ucTriangleArrows[1][3], ucTriangleArrows[1][4], ucTriangleArrows[1][5], true);
	}
}
