...
#define HRC6000_TASK_NAME "HRC6000Task"
#define HRC6000_TASK_STACK_SIZE (5000L / sizeof(portSTACK_TYPE))
#define HRC6000_TASK_PRIORITY (4U)
...
const uint8_t SILENCE_AUDIO[27] = {
	0xB9U, 0xE8U, 0x81U, 0x52U, 0x61U, 0x73U, 0x00U, 0x2AU, 0x6BU, 0xB9U, 0xE8U, 0x81U, 0x52U, 0x61U, 0x73U, 0x00U,
...
static const uint8_t spi_init_values_2[] = {
	0x69, 0x69, 0x96, 0x96, 0x96, 0x99, 0x99, 0x99, 0xa5, 0xa5, 0xaa, 0xaa, 0xcc, 0xcc, 0x00, 0xf0, 0x01, 0xff, 0x01,
	0x0f, 0x00, 0x00, 0x00, 0x00, 0x10, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};

// GD-77 FW V3.1.1 data from 0x75F90 / length 0x10
static const uint8_t spi_init_values_3[] = {
	0x00, 0x00, 0x14, 0x1e, 0x1a, 0xff, 0x3d, 0x50, 0x07, 0x60, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};
...
static const uint8_t spi_init_values_6[] = {
	0x32, 0xef, 0x00, 0x31, 0xef, 0x00, 0x12, 0xef, 0x00, 0x13, 0xef, 0x00, 0x14, 0xef, 0x00, 0x15, 0xef, 0x00, 0x16,
	0xef, 0x00, 0x17, 0xef, 0x00, 0x18, 0xef, 0x00, 0x19, 0xef, 0x00, 0x1a, 0xef, 0x00, 0x1b, 0xef, 0x00, 0x1c, 0xef,
	0x00, 0x1d, 0xef, 0x00, 0x1e, 0xef, 0x00, 0x1f, 0xef, 0x00, 0x20, 0xef, 0x00, 0x21, 0xef, 0x00, 0x22, 0xef, 0x00,
	0x23, 0xef, 0x00, 0x24, 0xef, 0x00, 0x25, 0xef, 0x00, 0x26, 0xef, 0x00, 0x27, 0xef, 0x00, 0x28, 0xef, 0x00, 0x29,
	0xef, 0x00, 0x2a, 0xef, 0x00, 0x2b, 0xef, 0x00, 0x2c, 0xef, 0x00, 0x2d, 0xef, 0x00, 0x2e, 0xef, 0x00, 0x2f, 0xef,
	0x00
};
...
static const uint8_t spiInitReg0x04MultiInit[57][2] = {
	...
	{ 0x1F, 0x10 },		/* Set LocalEMB DMR Color code in upper 4 bits - defaulted to 1, and is updated elsewhere in the code */
...
inline static void HRC6000SysSendStartInt(void) {
	...
	/**
	 * The sub-status registers indicate seven interrupts that initiate the transmission, including:
	 * + Bit7: Voice transmission starts
	 * + Bit6: OACSU requests to send interrupts, including first-time send and resend requests.
	 * + Bit5: End-to-end voice enhanced encryption interrupt, including EMB72bits update interrupt and voice 216bits
	 *         key update interrupt, which are distinguished by Bit5~Bit4 of Register 0x88, where 01 indicates EMB72bits
	 *         update interrupt and 10 indicates voice 216bits key update interrupt.
	 * + Bit4: The vocoder configuration returns an interrupt (this interrupt is sent by the HR_C6000 to the MCU when
	 *         the MCU manually configures the AMBE3000). This interrupt is only valid when using the external AMBE3000
	 *         vocoder.
	 * + Bit3: Data transmission starts
	 * + Bit2: Data partial retransmission
	 * + Bit1: Data retransmission
	 * + Bit0: The vocoder is initialized to an interrupt. This interrupt is only valid when using an external AMBE3000
	 *         or AMBE1000 vocoder.
	 *
...
inline static void HRC6000SysReceivedDataInt(void) {
	 ...
	 *
	 * In DMR mode, this interrupt has no sub-status register, but the error and receive type of its received data is
	 * given by the 0x51 register. The DLLRecvDataType, DLLRecvCRC are used to indicate the received data type and the
	 * error status, and the MCU accordingly performs the corresponding status.
	 *
	 * Display, you can also block the corresponding interrupt.
	 * In MSK mode, this interrupt has no substate interrupts.
	 *
	 * The FMB frame's EMB information parsing completion prompt is also the completion of the interrupt, which is
	 * distinguished by judging the 0x51 register SyncClass=0.
	 */
	int rxDataType;
	int rxSyncClass;
	int rpi;
	...
	if ((rxCRCisValid == true) && (rpi == 0) && (slot_state < DMR_STATE_TX_START_1)) {
		...
		if (slot_state == DMR_STATE_IDLE) {
			...
			if (((skip_count == 0) || ((receivedSrcId != trxDMRID) && (receivedSrcId != 0x00))) && (rxSyncClass != SYNC_CLASS_DATA) && (sequenceNumber >= 0x01) && (sequenceNumber <= 0x06) && (((trxDMRModeRx == DMR_MODE_RMO) && (checkTimeSlotFilter() && lastTimeCode != timeCode)) || ((trxDMRModeRx == DMR_MODE_DMO) && (slot_state == DMR_STATE_RX_1))) && checkTalkGroupFilter() && checkColourCodeFilter()) {
				ccHold = true;				//don't allow CC to change if we are receiving audio

				if ((settingsUsbMode != USB_MODE_HOTSPOT) && ((getAudioAmpStatus() & AUDIO_AMP_MODE_RF) == 0) && (((trxDMRModeRx == DMR_MODE_RMO) && (tsLockCount > 4)) || (trxDMRModeRx == DMR_MODE_DMO))) {
...
inline static void HRC6000SysInterruptHandler(void) {
	...
	if (!trxTransmissionEnabled) { // ignore the LC data when we are transmitting
		...
		if (rxCRCStatus && ((LCBuf[0] == TG_CALL_FLAG) || (LCBuf[0] == PC_CALL_FLAG) || ((LCBuf[0] >= FLCO_TALKER_ALIAS_HEADER) && (LCBuf[0] <= FLCO_GPS_INFO))) && (memcmp((uint8_t*)previousLCBuf, LCBuf, 12) != 0)) {
...
static void HRC6000TransitionToTx(void) {
	...
	SPI0WritePageRegByte(0x04, 0x22, 0x86);		/* Start Vocoder Encode, I²S mode */
...
void init_hrc6000_task(void) {
	xTaskCreate(fw_hrc6000_task, HRC6000_TASK_NAME, HRC6000_TASK_STACK_SIZE, NULL, HRC6000_TASK_PRIORITY, fwhrc6000TaskHandle);
}
