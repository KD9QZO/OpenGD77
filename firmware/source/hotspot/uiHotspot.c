 ...
 * It sends invalid data from time to time (that's not critical, it's simply rejected), but once it's heard a PC or TG call (from other
 * transceiver), it will keep repeating that, instead of invalid data.
 *
 * After investigation, it's turns out if you enable the 'GPS System' feature (even with correct configuration, like
 * GC xxx999 for BM), it will send such weird or echo data at each GPS interval.
 *
 *
 * ===== TL;DR: turn off GPS stuff on your MD-390 if it's listening local transmissions from other transceivers. =====
 */
...
static bool handleEvent(uiEvent_t *ev) {
	...
	if (KEYCHECK_SHORTUP(ev->keys, KEY_RED) || ((ev->events & BUTTON_EVENT) && (ev->buttons & BUTTON_ORANGE))) {
#else
...
static void hotspotSendVoiceFrame(volatile const uint8_t *receivedDMRDataAndAudio) {
	uint8_t frameData[DMR_FRAME_LENGTH_BYTES + MMDVM_HEADER_LENGTH + 2U] = {
		MMDVM_FRAME_START,
		(DMR_FRAME_LENGTH_BYTES + MMDVM_HEADER_LENGTH + 2U),
		MMDVM_DMR_DATA2
	};
...
static void sendVoiceHeaderLC_Frame(volatile const uint8_t *receivedDMRDataAndAudio) {
	uint8_t frameData[DMR_FRAME_LENGTH_BYTES + MMDVM_HEADER_LENGTH + 2U] = {
		...
		(DMR_FRAME_LENGTH_BYTES + MMDVM_HEADER_LENGTH + 2U),
		MMDVM_DMR_DATA2,
		(DMR_SYNC_DATA | DT_VOICE_LC_HEADER)
	};
...
static void sendTerminator_LC_Frame(volatile const uint8_t *receivedDMRDataAndAudio) {
	uint8_t frameData[DMR_FRAME_LENGTH_BYTES + MMDVM_HEADER_LENGTH + 2U] = {
		...
		(DMR_FRAME_LENGTH_BYTES + MMDVM_HEADER_LENGTH + 2U),
		MMDVM_DMR_DATA2,
		(DMR_SYNC_DATA | DT_TERMINATOR_WITH_LC)
	};
	DMRLC_T lc;
...
static bool getEmbeddedData(volatile const uint8_t *com_requestbuffer) {
	...
	/* the following are used for fake TA data */
...
#if (defined(HOTSPOT_TESTING) && defined(DEBUG))
		int flco = DMREmbeddedData_getFLCO();

		switch (flco) {
			case FLCO_GROUP:
				DMREmbeddedData_getLC(&lc);
#if (defined(DEBUG) && defined(SEGGER_RTT_ENABLED))
				SEGGER_RTT_printf(0, "Emb Group\tFID: %d\tFLCO: %d\tPF: %d\tR: %d\tdstId: %d\tsrcId: %d\topts: 0x%02X\n", lc.FID, lc.FLCO, lc.PF, lc.R, lc.dstId, lc.srcId, lc.options);
			...
			case FLCO_USER_USER:
				DMREmbeddedData_getLC(&lc);
#if (defined(DEBUG) && defined(SEGGER_RTT_ENABLED))
				SEGGER_RTT_printf(0, "Emb User\tFID: %d\tFLCO: %d\tPF: %d\tR: %d\tdstId: %d\tsrcId: %d\topts:0x%02X\n", lc.FID, lc.FLCO, lc.PF, lc.R, lc.dstId, lc.srcId, lc.options);
				displayDataBytes(hotspotTxLC, 9);
#endif
				break;
...
static void storeNetFrame(volatile const uint8_t *com_requestbuffer) {
	...
	if ((foundEmbedded || (nonVolatileSettings.hotspotType == HOTSPOT_TYPE_BLUEDV)) && (hotspotTxLC[0] == TG_CALL_FLAG || hotspotTxLC[0] == PC_CALL_FLAG) && (hotspotState != HOTSPOT_STATE_TX_START_BUFFERING && hotspotState != HOTSPOT_STATE_TRANSMITTING)) {
		timeoutCounter = TX_BUFFERING_TIMEOUT;  // set buffering timeout
		hotspotState = HOTSPOT_STATE_TX_START_BUFFERING;
	}

	if (hotspotState == HOTSPOT_STATE_TRANSMITTING || hotspotState == HOTSPOT_STATE_TX_SHUTDOWN || hotspotState == HOTSPOT_STATE_TX_START_BUFFERING) {
	...
	switch (hotspotState) {
		case HOTSPOT_STATE_NOT_CONNECTED:
			...
			if (mmdvmHostIsConnected) {
				...
				if ((nonVolatileSettings.hotspotType == HOTSPOT_TYPE_MMDVM) && ((fw_millis() - mmdvmHostLastActiveTime) > MMDVMHOST_TIMEOUT)) {
					wavbuffer_count = 0;
		...
		case HOTSPOT_STATE_RX_PROCESS:
			...
			if (rfFrameBufCount > 0) {
				/*
				 * We have pending data in RF side, but don't process it when MMDVMHost sets the hotspot in POCSAG mode;
				 * just trash it.
				 */
				if (modemState == STATE_POCSAG) {
				...
				if (((lastRxState == HOTSPOT_RX_AUDIO_FRAME) || (lastRxState == HOTSPOT_RX_START)) && ((fw_millis() - rxFrameTime) > 300)) { // 300ms
					sendDMRLost();
					updateScreen(HOTSPOT_RX_IDLE);
					lastRxState = HOTSPOT_RX_STOP;
					hotspotState = HOTSPOT_STATE_RX_END;
		...
		case HOTSPOT_STATE_TX_SHUTDOWN:
			...
			if (txstopdelay > 0) {
				...
				if ((trxIsTransmitting) || ((modemState == STATE_IDLE) && trxTransmissionEnabled)) { // MMDVMHost asked to go back to IDLE (mostly on shutdown)
					trxTransmissionEnabled = false;
					trxDisableTransmission();
					hotspotState = HOTSPOT_STATE_RX_START;
					updateScreen(HOTSPOT_RX_IDLE);
...
static void getStatus(void) {
	...
	buf[4U] = modemState;
	buf[5U] =
			(((hotspotState == HOTSPOT_STATE_TX_START_BUFFERING) || (hotspotState == HOTSPOT_STATE_TRANSMITTING) || (hotspotState == HOTSPOT_STATE_TX_SHUTDOWN)) || cwKeying) ? 0x01U : 0x00U;
	...
	if (com_requestbuffer[0] == MMDVM_FRAME_START) {
		...
		switch (com_requestbuffer[2U]) {
			...
			case MMDVM_SET_MODE: {
				...
				if (((prevState == STATE_POCSAG) && (modemState != STATE_POCSAG)) || ((prevState != STATE_POCSAG) && (modemState == STATE_POCSAG))) {
