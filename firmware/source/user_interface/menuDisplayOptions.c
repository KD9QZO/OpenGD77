		...
		if ((settingOption == false) || (i == 0)) {
			...
			switch (mNum) {
				...
				case DISPLAY_MENU_BACKLIGHT_MODE: {
					const char *const*backlightModes[] = {
						&currentLanguage->Auto, &currentLanguage->squelch, &currentLanguage->manual,
						&currentLanguage->buttons, &currentLanguage->none };
				...
				case DISPLAY_MENU_COLOUR_INVERT:
					...
					rightSideConst =
							settingsIsOptionBitSet(BIT_INVERSE_VIDEO) ? (char* const*)&currentLanguage->colour_invert : (char* const*)&currentLanguage->colour_normal;
					break;
				...
				case DISPLAY_MENU_CONTACT_DISPLAY_SPLIT_CONTACT: {
					const char *const*splitContact[] = {
						&currentLanguage->one_line, &currentLanguage->two_lines, &currentLanguage->Auto };
				...
				case DISPLAY_EXTENDED_INFOS: {
					const char *const*extendedInfos[] = {
						&currentLanguage->off, &currentLanguage->ts, &currentLanguage->pwr, &currentLanguage->both };
				...
				case DISPLAY_ALL_LEDS_ENABLED:
					...
					rightSideConst =
							settingsIsOptionBitSet(BIT_ALL_LEDS_DISABLED) ? (char* const*)&currentLanguage->off : (char* const*)&currentLanguage->on;
					break;
			}
	...
	if ((ev->events & KEY_EVENT) && (menuDataGlobal.menuOptionsSetQuickkey == 0) && (menuDataGlobal.menuOptionsTimeout == 0)) {
		if (KEYCHECK_PRESS(ev->keys, KEY_DOWN) && (menuDataGlobal.endIndex != 0)) {
			...
			if ((nonVolatileSettings.bitfieldOptions & BIT_ALL_LEDS_DISABLED) != (originalNonVolatileSettings.bitfieldOptions & BIT_ALL_LEDS_DISABLED)) {
				...
				GPIO_PinWrite(GPIO_LEDgreen, Pin_LEDgreen, (
						(nonVolatileSettings.bitfieldOptions & BIT_ALL_LEDS_DISABLED) ? 0 : state));
			}
	...
	if ((ev->events & (KEY_EVENT | FUNCTION_EVENT)) && (menuDataGlobal.menuOptionsSetQuickkey == 0)) {
		...
		if (KEYCHECK_PRESS(ev->keys, KEY_RIGHT) || (QUICKKEY_FUNCTIONID(ev->function) == FUNC_RIGHT)) {
			...
			switch (menuDataGlobal.currentItemIndex) {
				case DISPLAY_MENU_BRIGHTNESS:
					settingsIncrement(nonVolatileSettings.displayBacklightPercentage, (int8_t ) (
				...
				case DISPLAY_MENU_BRIGHTNESS_OFF:
					if (nonVolatileSettings.displayBacklightPercentageOff < nonVolatileSettings.displayBacklightPercentage) {
						settingsIncrement(nonVolatileSettings.displayBacklightPercentageOff, (int8_t ) (
			...
			switch (menuDataGlobal.currentItemIndex) {
				case DISPLAY_MENU_BRIGHTNESS:
					settingsDecrement(nonVolatileSettings.displayBacklightPercentage, (int8_t ) (
					...
					break;
				case DISPLAY_MENU_BRIGHTNESS_OFF:
					settingsDecrement(nonVolatileSettings.displayBacklightPercentageOff, (int8_t ) (
							(nonVolatileSettings.displayBacklightPercentageOff <= BACKLIGHT_PERCENTAGE_STEP) ? BACKLIGHT_PERCENTAGE_STEP_SMALL : BACKLIGHT_PERCENTAGE_STEP));
				...
				case DISPLAY_MENU_BACKLIGHT_MODE:
					...
					break;
				case DISPLAY_MENU_TIMEOUT:
					if (((nonVolatileSettings.backlightMode == BACKLIGHT_MODE_AUTO) && (nonVolatileSettings.backLightTimeout >= BACKLIGHT_TIMEOUT_STEP)) || (((nonVolatileSettings.backlightMode == BACKLIGHT_MODE_SQUELCH) || (nonVolatileSettings.backlightMode == BACKLIGHT_MODE_BUTTONS)) && (nonVolatileSettings.backLightTimeout >= (BACKLIGHT_TIMEOUT_STEP * 2)))) {
...
static void checkMinBacklightValue(void) {
	if (nonVolatileSettings.displayBacklightPercentageOff >= nonVolatileSettings.displayBacklightPercentage) {
		settingsSet(nonVolatileSettings.displayBacklightPercentageOff, (int8_t ) (
				nonVolatileSettings.displayBacklightPercentage ? (nonVolatileSettings.displayBacklightPercentage - (
						(nonVolatileSettings.displayBacklightPercentageOff <= BACKLIGHT_PERCENTAGE_STEP) ? BACKLIGHT_PERCENTAGE_STEP_SMALL : BACKLIGHT_PERCENTAGE_STEP)) : 0));
	}
}
