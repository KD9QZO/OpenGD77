...
/**
 * \brief update a temporary copy of the channel and only write back if green menu is pressed
 */
...
/**
 * \brief Enumerates the items that are displayed in the channel details list
 *
 * \note The last item in the list is used so that we automatically get a total number of items in the list.
 */
enum CHANNEL_DETAILS_DISPLAY_LIST {
	CH_DETAILS_NAME = 0,	/*!< The text label ("name") of the channel */
	CH_DETAILS_RXFREQ,		/*!< The \b receive frequency for the channel */
	CH_DETAILS_TXFREQ,		/*!< The \b transmit frequency for the channel */
	CH_DETAILS_MODE,		/*!< The \b mode in which the channel operates: \b DMR or \b ANALOG */
	CH_DETAILS_DMR_CC,		/*!< The <strong>color code</strong> for \b DMR channels */
	CH_DETAILS_DMR_TS,		/*!< The \b timeslot for \b DMR channels */
	CH_DETAILS_RXGROUP,		/*!< The <strong>receive group</strong> for \b DMR channels */
	CH_DETAILS_RXCSS,		/*!< The \e receive \b CTCSS or \b DCS code for \b ANALOG channels */
	CH_DETAILS_TXCSS,		/*!< The \e transmit \b CTCSS or \b DCS code for \b ANALOG channels */
	CH_DETAILS_BANDWIDTH,	/*!< The \b bandwidth of an \b ANALOG channel: \b 12.5kHz (narrow) or \b 25kHz (wide) */
	CH_DETAILS_FREQ_STEP,	/*!< The frequency step that should be used */
	CH_DETAILS_TOT,			/*!< The \b time-out for transmission */
	CH_DETAILS_RXONLY,		/*!< Whether the channel is \b receive-only */
	CH_DETAILS_ZONE_SKIP,	/*!< Whether the channel should be skipped during \b zone scans */
	CH_DETAILS_ALL_SKIP,	/*!< Whether the channel should be skipped during \b all scans */
	CH_DETAILS_VOX,			/*!< Whether \b VOX is enabled on the channel */
	CH_DETAILS_POWER,		/*!< The <strong>transmit power</strong> to use for the channel */
	CH_DETAILS_SQUELCH,		/*!< The \b squelch level to use for analog channels */
	NUM_CH_DETAILS_ITEMS	/*!< The last item in the list; used to automatically get the number of items in the list */
};
...
static void updateScreen(bool isFirstRun, bool allowedToSpeakUpdate) {
	...
	if (uiDataGlobal.FreqEnter.index != 0) {
		snprintf(buf, bufferLen, "%c%c%c.%c%c%c%c%c MHz", uiDataGlobal.FreqEnter.digits[0], uiDataGlobal.FreqEnter.digits[1], uiDataGlobal.FreqEnter.digits[2], uiDataGlobal.FreqEnter.digits[3], uiDataGlobal.FreqEnter.digits[4], uiDataGlobal.FreqEnter.digits[5], uiDataGlobal.FreqEnter.digits[6], uiDataGlobal.FreqEnter.digits[7]);
		...
		for (int i = -1; i <= 1; i++) {
			if ((settingOption == false) || (i == 0)) {
				...
				if (leftSide != NULL) {
					snprintf(buf, bufferLen, "%s:%s", *leftSide, (
							rightSideVar[0] ? rightSideVar : (rightSideConst ? *rightSideConst : "")));
				} else {
				...
				if ((i == 0) && allowedToSpeakUpdate) {
					...
					} else if ((mNum == CH_DETAILS_POWER) && ((tmpChannel.libreDMR_Power != 0) && (uiDataGlobal.currentSelectedChannelNumber != CH_DETAILS_VFO_CHANNEL))) {
						char buf2[bufferLen];
						char *p;
...
static void handleEvent(uiEvent_t *ev) {
	...
	if (ev->events & FUNCTION_EVENT) {
		...
		if ((QUICKKEY_FUNCTIONID(ev->function) != 0) && ((menuDataGlobal.currentItemIndex != CH_DETAILS_NAME) && (menuDataGlobal.currentItemIndex != CH_DETAILS_RXFREQ) && (menuDataGlobal.currentItemIndex != CH_DETAILS_TXFREQ))) {
		...
		if (KEYCHECK_LONGDOWN(ev->keys, KEY_RIGHT) || KEYCHECK_LONGDOWN_REPEAT(ev->keys, KEY_RIGHT)) {
			...
			switch (menuDataGlobal.currentItemIndex) {
				...
				case CH_DETAILS_RXCSS:
					if (tmpChannel.chMode == RADIO_MODE_ANALOG) {
						...
						announceCSSCode(tmpChannel.rxTone, RxCSSType, (
								((voicePromptWasPlaying == false) && (nonVolatileSettings.audioPromptMode > AUDIO_PROMPT_MODE_VOICE_LEVEL_2)) ? DIRECTION_RECEIVE : DIRECTION_NONE), (voicePromptWasPlaying == false), AUDIO_PROMPT_MODE_VOICE_LEVEL_1);
						allowedToSpeakUpdate = false;
					}
					break;
				case CH_DETAILS_TXCSS:
					if (tmpChannel.chMode == RADIO_MODE_ANALOG) {
						...
						announceCSSCode(tmpChannel.txTone, TxCSSType, (
								((voicePromptWasPlaying == false) && (nonVolatileSettings.audioPromptMode > AUDIO_PROMPT_MODE_VOICE_LEVEL_2)) ? DIRECTION_TRANSMIT : DIRECTION_NONE), (voicePromptWasPlaying == false), AUDIO_PROMPT_MODE_VOICE_LEVEL_1);
						allowedToSpeakUpdate = false;
					}
					break;
				case CH_DETAILS_BANDWIDTH:
				...
				case CH_DETAILS_VOX:
					...
					break;
				case CH_DETAILS_POWER:
					if ((uiDataGlobal.currentSelectedChannelNumber != CH_DETAILS_VFO_CHANNEL) && (tmpChannel.libreDMR_Power < (MAX_POWER_SETTING_NUM + CODEPLUG_MIN_PER_CHANNEL_POWER))) {
			...
			switch (menuDataGlobal.currentItemIndex) {
				...
				case CH_DETAILS_VOX:
					...
					break;
				case CH_DETAILS_POWER:
					if ((uiDataGlobal.currentSelectedChannelNumber != CH_DETAILS_VFO_CHANNEL) && (tmpChannel.libreDMR_Power > 0)) {
						tmpChannel.libreDMR_Power--;
					}
					break;
				case CH_DETAILS_SQUELCH:
