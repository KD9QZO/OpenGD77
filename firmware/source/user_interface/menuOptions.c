...
enum OPTIONS_MENU_LIST {
	OPTIONS_MENU_TX_FREQ_LIMITS = 0U,
		...
		if ((settingOption == false) || (i == 0)) {
			...
			switch (mNum) {
				...
				case OPTIONS_MENU_SCAN_MODE:  // scanning mode
					...
					{
						const char *const*scanModes[] = {
							&currentLanguage->hold, &currentLanguage->pause, &currentLanguage->stop };
				...
				case OPTIONS_MENU_PTT_TOGGLE:
					...
					rightSideConst = (char* const*)(
							settingsIsOptionBitSet(BIT_PTT_LATCH) ? &currentLanguage->on : &currentLanguage->off);
					break;
				...
				case OPTIONS_MENU_TALKER_ALIAS_TX:
					...
					rightSideConst = (char* const*)(
							settingsIsOptionBitSet(BIT_TRANSMIT_TALKER_ALIAS) ? &currentLanguage->on : &currentLanguage->off);
					break;
				case OPTIONS_MENU_PRIVATE_CALLS:
					leftSide = (char* const*)&currentLanguage->private_call_handling;
					const char *const*allowPCOptions[] = {
						&currentLanguage->off, &currentLanguage->on, &currentLanguage->ptt, &currentLanguage->Auto };
