...
int8_t tsGetManualOverrideFromCurrentChannel(void) {
	Channel_t chan = (
			((currentChannelData->NOT_IN_CODEPLUG_flag & 0x01) == 0x01) ? (
...
bool isQSODataAvailableForCurrentTalker(void) {
	...
	if ((trxTransmissionEnabled == false) && ((trxGetMode() == RADIO_MODE_DIGITAL) && (rxID != 0) && (HRC6000GetReceivedTgOrPcId() != 0)) && (getAudioAmpStatus() & AUDIO_AMP_MODE_RF) && checkTalkGroupFilter() && (((item = lastheardFindInList(rxID)) != NULL) && (item == LinkHead))) {
		return true;
	}
	...
	if ((HRC6000GetReceivedTgOrPcId() != 0) || forceOnHotspot) {
		if (dmrDataBuffer[0] == TG_CALL_FLAG || dmrDataBuffer[0] == PC_CALL_FLAG) {
			...
			if (blockID < 4) {
				...
				if ((blockID == 0) && ((blocksTA & (1 << blockID)) != 0) && (bufferTA[0] != (
						forceOnHotspot ? dmrDataBuffer[2] : DMR_frame_buffer[2]))) {
					blocksTA &= ~(1 << 0);
			...
			{
				uint8_t *locator = decodeGPSPosition((uint8_t*)(
						forceOnHotspot ? &dmrDataBuffer[0] : &DMR_frame_buffer[0]));
	...
	if ((dmrIDsCache.entries > 0) && (targetIdBCD >= dmrIDsCache.slices[0]) && (targetIdBCD <= dmrIDsCache.slices[ID_SLICES - 1])) {
		...
		} else  // Not enough contact to use slices
		{
			...
			if ((isMin = (targetIdBCD == dmrIDsCache.slices[0])) || (targetIdBCD == dmrIDsCache.slices[ID_SLICES - 1])) {
				...
				dmrIDReadContactInFlash((dmrIDsCache.contactLength * (dmrIDsCache.IDsPerSlice * (
						isMin ? 0 : (ID_SLICES - 1)))) + 4U, (uint8_t*)foundRecord + 4U, (dmrIDsCache.contactLength - 4U));
...
static void displayContactTextInfos(char *text, size_t maxLen, bool isFromTalkerAlias) {
	...
	if (strlen(text) >= 5 && isFromTalkerAlias)  // if it's Talker Alias and there is more text than just the callsign, split across 2 lines
	{
		...
		if ((nonVolatileSettings.splitContact == SPLIT_CONTACT_SINGLE_LINE_ONLY) || ((nonVolatileSettings.splitContact == SPLIT_CONTACT_AUTO) && (strlen(text) <= 16))) {
	...
	switch (line) {
		...
		case DISPLAY_INFO_TONE_AND_SQUELCH: {
			...
			if (trxGetMode() == RADIO_MODE_ANALOG) {
				...
				snprintf(buf, 24, "SQL:%d%%", 5 * ((
						(currentChannelData->sql == 0) ? nonVolatileSettings.squelchDefaults[trxCurrentBand[TRX_RX_FREQ_BAND]] : currentChannelData->sql) - 1));
...
void uiUtilityRenderQSOData(void) {
	...
	if (HRC6000GetReceivedTgOrPcId() != 0) {
		if ((LinkHead->talkGroupOrPcId >> 24) == PC_CALL_FLAG)  // &&  (LinkHead->id & 0xFFFFFF) != (trxTalkGroupOrPcId & 0xFFFFFF))
		{
			...
			// Group call
			bool different = (((LinkHead->talkGroupOrPcId & 0xFFFFFF) != trxTalkGroupOrPcId) || (((trxDMRModeRx != DMR_MODE_DMO) && (dmrMonitorCapturedTS != -1)) && (dmrMonitorCapturedTS != trxGetDMRTimeSlot())) || (trxGetDMRColourCode() != currentChannelData->txColor));

			uiUtilityDisplayInformation(LinkHead->talkgroup,
					different ? DISPLAY_INFO_CONTACT_INVERTED : DISPLAY_INFO_CONTACT, -1);
	...
	switch (trxGetMode()) {
		case RADIO_MODE_ANALOG:
			...
			ucPrintCore(MODE_TEXT_X_OFFSET, DISPLAY_Y_POS_HEADER, buffer, (
					(nonVolatileSettings.hotspotType != HOTSPOT_TYPE_OFF) ? FONT_SIZE_1_BOLD : FONT_SIZE_1), TEXT_ALIGN_LEFT, scanBlinkPhase);

			if ((monitorModeData.isEnabled == false) && ((currentChannelData->txTone != CODEPLUG_CSS_NONE) || (currentChannelData->rxTone != CODEPLUG_CSS_NONE))) {
		...
		case RADIO_MODE_DIGITAL:
			if (settingsUsbMode != USB_MODE_HOTSPOT) {
				...
				if (nonVolatileSettings.extendedInfosOnScreen & (INFO_ON_SCREEN_TS & INFO_ON_SCREEN_BOTH)) {
					...
					tsManOverride = (
							contactTSActive ? tsIsContactHasBeenOverriddenFromCurrentChannel() : (tsGetManualOverrideFromCurrentChannel() != 0));
				}
				...
				if (!scanBlinkPhase) {
					bool isInverted =
							isVFODualWatchScanning ? false : (scanBlinkPhase ^ (nonVolatileSettings.dmrDestinationFilter > DMR_DESTINATION_FILTER_NONE));
					ucPrintCore(MODE_TEXT_X_OFFSET, DISPLAY_Y_POS_HEADER, isVFODualWatchScanning ? "[DW]" : "DMR", (
							(nonVolatileSettings.hotspotType != HOTSPOT_TYPE_OFF) ? FONT_SIZE_1_BOLD : FONT_SIZE_1), TEXT_ALIGN_LEFT, isInverted);
				}
				...
				if (isVFODualWatchScanning == false) {
					...
					ucPrintCore(FILTER_TEXT_X_OFFSET, DISPLAY_Y_POS_HEADER, buffer, (
							tsManOverride ? FONT_SIZE_1_BOLD : FONT_SIZE_1), TEXT_ALIGN_LEFT, tsInverted);
				}
			}
			break;
	}
...
void uiUtilityDisplayFrequency(uint8_t y, bool isTX, bool hasFocus, uint32_t frequency, bool displayVFOChannel, bool isScanMode, uint8_t dualWatchVFO) {
	...
	if (displayVFOChannel) {
		ucPrintAt(16, y + VFO_LETTER_Y_OFFSET,
				(((dualWatchVFO == 0) && (nonVolatileSettings.currentVFONumber == 0)) || (dualWatchVFO == 1)) ? "A" : "B", FONT_SIZE_1);
	}
	// Frequency
...
bool increasePowerLevel(bool allowFullPower) {
	...
	if (currentChannelData->libreDMR_Power != 0x00) {
		if (currentChannelData->libreDMR_Power < (MAX_POWER_SETTING_NUM - 1 + CODEPLUG_MIN_PER_CHANNEL_POWER) + (
				allowFullPower ? 1 : 0)) {
...
void announceCSSCode(uint16_t code, CSSTypes_t cssType, Direction_t direction, bool announceType, audioPromptThreshold_t immediateAnnounceThreshold) {
	...
	if ((voicePromptWasPlaying && (nonVolatileSettings.audioPromptMode >= AUDIO_PROMPT_MODE_VOICE_LEVEL_2)) || (nonVolatileSettings.audioPromptMode >= immediateAnnounceThreshold)) {
		voicePromptsPlay();
	}
}
...
void acceptPrivateCall(int id, int timeslot) {
	...
	if (timeslot != trxGetDMRTimeSlot()) {
		...
		tsSetManualOverride((
				(menuSystemGetRootMenuNumber() == UI_CHANNEL_MODE) ? CHANNEL_CHANNEL : (CHANNEL_VFO_A + nonVolatileSettings.currentVFONumber)), (timeslot + 1));
	}
#else
	(void)timeslot;
#endif
...
bool repeatVoicePromptOnSK1(uiEvent_t *ev) {
	if (BUTTONCHECK_SHORTUP(ev, BUTTON_SK1) && (ev->keys.key == 0)) {
		if (nonVolatileSettings.audioPromptMode >= AUDIO_PROMPT_MODE_VOICE_LEVEL_1) {
			...
			if ((((currentMenu == UI_CHANNEL_MODE) && (uiDataGlobal.Scan.active && (uiDataGlobal.Scan.state != SCAN_PAUSED))) || ((currentMenu == UI_VFO_MODE) && ((uiDataGlobal.Scan.active && (uiDataGlobal.Scan.state != SCAN_PAUSED)) || uiDataGlobal.Scan.toneActive))) == false) {
...
static uint32_t dtmfGetToneDuration(uint32_t duration) {
	...
	if ((uiDataGlobal.DTMFContactList.poPtr == 0) && (uiDataGlobal.DTMFContactList.durations.fstDur > 0)) {
		 ...
		 */
		return ((
				starOrHash ? (uiDataGlobal.DTMFContactList.durations.otherDur * 100) : (uiDataGlobal.DTMFContactList.durations.fstDur * 100)) + duration);
	}
