...
static vfoScreenOperationMode_t screenOperationMode[2] = {
	VFO_SCREEN_OPERATION_NORMAL,
	VFO_SCREEN_OPERATION_NORMAL
};	/* For VFO A and B */
...
menuStatus_t uiVFOMode(uiEvent_t *ev, bool isFirstRun) {
	...
	if (isFirstRun) {
		...
		uiDataGlobal.displayQSOState = (
				isQSODataAvailableForCurrentTalker() ? QSO_DISPLAY_CALLER_DATA : QSO_DISPLAY_DEFAULT_SCREEN);
		...
		if (!uiDataGlobal.VoicePrompts.inhibitInitial) {
			...
			announceItem(PROMPT_SEQUENCE_CHANNEL_NAME_AND_CONTACT_OR_VFO_FREQ_AND_MODE,
					((nextMenu == UI_TX_SCREEN) || (nextMenu == UI_PRIVATE_CALL)) ? PROMPT_THRESHOLD_NEVER_PLAY_IMMEDIATELY : PROMPT_THRESHOLD_3);
		}
		...
		if (ev->events == NO_EVENT) {
			...
			if (ev->hasEvent) {
				if ((currentChannelData->chMode == RADIO_MODE_ANALOG) && (ev->events & KEY_EVENT) && ((ev->keys.key == KEY_LEFT) || (ev->keys.key == KEY_RIGHT))) {
					sqm = ev->time;
				}
				...
				if (uiDataGlobal.Scan.toneActive) {
					...
					if (((ev->events & BUTTON_EVENT) && BUTTONCHECK_SHORTUP(ev, BUTTON_ORANGE)) || ((ev->keys.key != 0) && (ev->keys.event & KEY_MOD_UP)))
#endif
							{
...
void uiVFOModeUpdateScreen(int txTimeSecs) {
	...
	if ((trxGetMode() == RADIO_MODE_DIGITAL) && (HRC6000GetReceivedTgOrPcId() == 0) && ((uiDataGlobal.displayQSOState == QSO_DISPLAY_CALLER_DATA) || (uiDataGlobal.displayQSOState == QSO_DISPLAY_CALLER_DATA_UPDATE))) {
	...
	if (uiDataGlobal.displayChannelSettings && ((uiDataGlobal.displayQSOState == QSO_DISPLAY_CALLER_DATA) || (uiDataGlobal.displayQSOState == QSO_DISPLAY_CALLER_DATA_UPDATE))) {
	...
	switch (uiDataGlobal.displayQSOState) {
		case QSO_DISPLAY_DEFAULT_SCREEN:
			if (uiDataGlobal.Scan.active && (screenOperationMode[nonVolatileSettings.currentVFONumber] == VFO_SCREEN_OPERATION_DUAL_SCAN) && (uiDataGlobal.Scan.state == SCAN_SCANNING)) {
				...
				if (trxGetMode() == RADIO_MODE_DIGITAL) {
					if (uiDataGlobal.displayChannelSettings) {
						uint32_t PCorTG = (
								(nonVolatileSettings.overrideTG != 0) ? nonVolatileSettings.overrideTG : currentContactData.tgNumber);

						snprintf(buffer, bufferLen, "%s %d", (
								((PCorTG >> 24) == PC_CALL_FLAG) ? currentLanguage->pc : currentLanguage->tg), (PCorTG & 0xFFFFFF));
					} else {
						if (nonVolatileSettings.overrideTG != 0) {
							uiUtilityBuildTgOrPCDisplayName(buffer, bufferLen);
							uiUtilityDisplayInformation(NULL, DISPLAY_INFO_CONTACT_OVERRIDE_FRAME, (
									trxTransmissionEnabled ? DISPLAY_Y_POS_CONTACT_TX_FRAME : -1));
						} else {
							codeplugUtilConvertBufToString(currentContactData.name, buffer, 16);
						}
					}

					uiUtilityDisplayInformation(buffer, DISPLAY_INFO_CONTACT, (
							trxTransmissionEnabled ? DISPLAY_Y_POS_CONTACT_TX : -1));
				} else {
				...
				if (uiDataGlobal.FreqEnter.index == 0) {
					if (!trxTransmissionEnabled) {
						...
						uiUtilityDisplayFrequency(DISPLAY_Y_POS_RX_FREQ, false, (selectedFreq == VFO_SELECTED_FREQUENCY_INPUT_RX), (
								uiDataGlobal.reverseRepeater ? currentChannelData->txFreq : currentChannelData->rxFreq), true, (screenOperationMode[nonVolatileSettings.currentVFONumber] == VFO_SCREEN_OPERATION_SCAN), 0);
					} else {
					...
					if (((screenOperationMode[nonVolatileSettings.currentVFONumber] == VFO_SCREEN_OPERATION_NORMAL) || (screenOperationMode[nonVolatileSettings.currentVFONumber] == VFO_SCREEN_OPERATION_DUAL_SCAN)) || trxTransmissionEnabled) {
						...
						static const int scanDirArrow[2][6] = { {  // Down
								59, (DISPLAY_Y_POS_TX_FREQ + (FONT_SIZE_3_HEIGHT / 2) - 1), 67,
								(DISPLAY_Y_POS_TX_FREQ + (FONT_SIZE_3_HEIGHT / 2) - (FONT_SIZE_3_HEIGHT / 4) - 1), 67,
								(DISPLAY_Y_POS_TX_FREQ + (FONT_SIZE_3_HEIGHT / 2) + (FONT_SIZE_3_HEIGHT / 4) - 1) },  // Up
							{
								59, (DISPLAY_Y_POS_TX_FREQ + (FONT_SIZE_3_HEIGHT / 2) + (FONT_SIZE_3_HEIGHT / 4) - 1),
								59, (DISPLAY_Y_POS_TX_FREQ + (FONT_SIZE_3_HEIGHT / 2) - (FONT_SIZE_3_HEIGHT / 4) - 1),
								67, (DISPLAY_Y_POS_TX_FREQ + (FONT_SIZE_3_HEIGHT / 2) - 1) } };

						ucFillTriangle(scanDirArrow[(uiDataGlobal.Scan.direction > 0)][0], scanDirArrow[(uiDataGlobal.Scan.direction > 0)][1], scanDirArrow[(uiDataGlobal.Scan.direction > 0)][2], scanDirArrow[(uiDataGlobal.Scan.direction > 0)][3], scanDirArrow[(uiDataGlobal.Scan.direction > 0)][4], scanDirArrow[(uiDataGlobal.Scan.direction > 0)][5], true);
					}
				} else  // Entering digits
				{
					...
					if ((screenOperationMode[nonVolatileSettings.currentVFONumber] == VFO_SCREEN_OPERATION_NORMAL) || (screenOperationMode[nonVolatileSettings.currentVFONumber] == VFO_SCREEN_OPERATION_DUAL_SCAN)) {
#if defined(PLATFORM_RD5R)
						...
						snprintf(buffer, bufferLen, FREQ_DISP_STR, uiDataGlobal.FreqEnter.digits[0], uiDataGlobal.FreqEnter.digits[1], uiDataGlobal.FreqEnter.digits[2], uiDataGlobal.FreqEnter.digits[3], uiDataGlobal.FreqEnter.digits[4], uiDataGlobal.FreqEnter.digits[5], uiDataGlobal.FreqEnter.digits[6], uiDataGlobal.FreqEnter.digits[7]);

						ucPrintCentered(
								(selectedFreq == VFO_SELECTED_FREQUENCY_INPUT_TX) ? DISPLAY_Y_POS_TX_FREQ : DISPLAY_Y_POS_RX_FREQ, buffer, FONT_SIZE_3);

						// Cursor
						if (uiDataGlobal.FreqEnter.index < 8) {
							xCursor = ((DISPLAY_SIZE_X - (strlen(buffer) * 8)) >> 1) + ((uiDataGlobal.FreqEnter.index + (
									(uiDataGlobal.FreqEnter.index > 2) ? 1 : 0)) * 8);
							yCursor = (
									(selectedFreq == VFO_SELECTED_FREQUENCY_INPUT_TX) ? DISPLAY_Y_POS_TX_FREQ : DISPLAY_Y_POS_RX_FREQ) + (FONT_SIZE_3_HEIGHT - 2);
						}
					} else {
						...
						sprintf(buffer, "%c%c%c.%c%c%c", uiDataGlobal.FreqEnter.digits[0], uiDataGlobal.FreqEnter.digits[1], uiDataGlobal.FreqEnter.digits[2], uiDataGlobal.FreqEnter.digits[3], uiDataGlobal.FreqEnter.digits[4], uiDataGlobal.FreqEnter.digits[5]);
						...
						sprintf(buffer, "%c%c%c.%c%c%c", uiDataGlobal.FreqEnter.digits[6], uiDataGlobal.FreqEnter.digits[7], uiDataGlobal.FreqEnter.digits[8], uiDataGlobal.FreqEnter.digits[9], uiDataGlobal.FreqEnter.digits[10], uiDataGlobal.FreqEnter.digits[11]);
...
static void checkAndFixIndexInRxGroup(void) {
	if ((currentRxGroupData.NOT_IN_CODEPLUG_numTGsInGroup > 0) && (nonVolatileSettings.currentIndexInTRxGroupList[SETTINGS_VFO_A_MODE + nonVolatileSettings.currentVFONumber] > (currentRxGroupData.NOT_IN_CODEPLUG_numTGsInGroup - 1))) {
...
static void handleEvent(uiEvent_t *ev) {
	if (uiDataGlobal.Scan.active && (ev->events & KEY_EVENT)) {
		...
		if (((ev->keys.key == KEY_UP) && (BUTTONCHECK_DOWN(ev, BUTTON_SK2) == 0) && (screenOperationMode[nonVolatileSettings.currentVFONumber] != VFO_SCREEN_OPERATION_DUAL_SCAN)) == false) {
			uiVFOModeStopScanning();
			keyboardReset();
	...
	if (ev->events & BUTTON_EVENT) {
		...
		if (uiDataGlobal.isDisplayingQSOData && BUTTONCHECK_SHORTUP(ev, BUTTON_SK2) && (trxGetMode() == RADIO_MODE_DIGITAL) && ((trxTalkGroupOrPcId != tg) || ((dmrMonitorCapturedTS != -1) && (dmrMonitorCapturedTS != trxGetDMRTimeSlot())) || (trxGetDMRColourCode() != currentChannelData->txColor))) {
			lastHeardClearLastID();
	...
	if (ev->events & KEY_EVENT) {
		...
		if (uiDataGlobal.FreqEnter.index == 0) {
			if (KEYCHECK_SHORTUP(ev->keys, KEY_HASH)) {
				...
				if (BUTTONCHECK_DOWN(ev, BUTTON_SK2)) {
					...
					if ((screenOperationMode[nonVolatileSettings.currentVFONumber] != VFO_SCREEN_OPERATION_SCAN) || (screenOperationMode[nonVolatileSettings.currentVFONumber] != VFO_SCREEN_OPERATION_DUAL_SCAN)) {
						...
						if (currentRxGroupData.NOT_IN_CODEPLUG_numTGsInGroup != 0) {
							if (currentRxGroupData.NOT_IN_CODEPLUG_numTGsInGroup > 1) {
								...
								if (nonVolatileSettings.overrideTG == 0) {
									...
									if (nonVolatileSettings.currentIndexInTRxGroupList[SETTINGS_VFO_A_MODE + nonVolatileSettings.currentVFONumber] < 0) {
										settingsSet(nonVolatileSettings.currentIndexInTRxGroupList[SETTINGS_VFO_A_MODE + nonVolatileSettings.currentVFONumber], (int16_t ) (currentRxGroupData.NOT_IN_CODEPLUG_numTGsInGroup - 1));
									}
		...
		if (uiDataGlobal.FreqEnter.index < (
				(screenOperationMode[nonVolatileSettings.currentVFONumber] == VFO_SCREEN_OPERATION_NORMAL) ? 8 : 12)) {
			int keyval = menuGetKeypadKeyValue(ev, true);

			if ((keyval != 99) &&
			// Not first '0' digit in frequencies: we don't support < 100 MHz
			((((uiDataGlobal.FreqEnter.index == 0) && (keyval == 0)) == false) && (((screenOperationMode[nonVolatileSettings.currentVFONumber] == VFO_SCREEN_OPERATION_SCAN) && (uiDataGlobal.FreqEnter.index == 6) && (keyval == 0)) == false)) && (BUTTONCHECK_DOWN(ev, BUTTON_SK2) == 0)) {
				voicePromptsInit();
...
static void stepFrequency(int increment) {
	...
	if (trxGetBandFromFrequency(newRxFreq) == -1) {
		...
		if (band != -1) {
			newRxFreq = (
					(increment > 0) ? RADIO_HARDWARE_FREQUENCY_BANDS[band].minFreq : RADIO_HARDWARE_FREQUENCY_BANDS[band].maxFreq);
			newTxFreq = newRxFreq;
		} else {
			soundSetMelody(MELODY_ERROR_BEEP);
			return;
...
enum VFO_SCREEN_QUICK_MENU_ITEMS // The last item in the list is used so that we automatically get a total number of items in the list
{
	...
	VFO_SCREEN_TONE_SCAN,
	VFO_SCREEN_DUAL_SCAN,
	...
	for (int i = -1; i <= 1; i++) {
		...
		switch (mNum) {
			...
			case VFO_SCREEN_QUICK_MENU_DMR_CC_FILTER:
				...
				rightSideConst =
						(uiDataGlobal.QuickMenu.tmpDmrCcTsFilterLevel & DMR_CC_FILTER_PATTERN) ? (char* const*)&currentLanguage->on : (char* const*)&currentLanguage->off;
				break;
			case VFO_SCREEN_QUICK_MENU_DMR_TS_FILTER:
				leftSide = (char* const*)&currentLanguage->dmr_ts_filter;
				rightSideConst =
						(uiDataGlobal.QuickMenu.tmpDmrCcTsFilterLevel & DMR_TS_FILTER_PATTERN) ? (char* const*)&currentLanguage->on : (char* const*)&currentLanguage->off;
				break;
			...
			case VFO_SCREEN_QUICK_MENU_FREQ_BIND_MODE:
				...
				rightSideConst =
						(!uiDataGlobal.QuickMenu.tmpTxRxLockMode) ? (char* const*)&currentLanguage->on : (char* const*)&currentLanguage->off;
				break;
			default:
				strcpy(buf, "");
				break;
		...
		switch (menuDataGlobal.currentItemIndex) {
			...
			case VFO_SCREEN_TONE_SCAN:
				if (trxGetMode() == RADIO_MODE_ANALOG) {
					...
					uint16_t currentCSSType = (
							codeplugChannelToneIsCTCSS(currentChannelData->rxTone) ? CSS_CTCSS : (
					...
					//                                          CTCSS or DCS         no CSS
					toneScanType = (
							(toneScanCSS != CSS_NONE) ? toneScanCSS : (
									(currentCSSType == CSS_NONE) ? CSS_CTCSS : currentCSSType));
					...
					uiDataGlobal.Scan.timer = (
							(toneScanType == CSS_CTCSS) ? (SCAN_TONE_INTERVAL - (scanToneIndex * 2)) : SCAN_TONE_INTERVAL);
				}
				break;
			case VFO_SCREEN_DUAL_SCAN:
				uiDataGlobal.Scan.active = true;
...
static void scanning(void) {
	...
	if (uiDataGlobal.Scan.timer > 0) {
		...
		{
			uiEvent_t tmpEvent = {
				.buttons = 0, .keys = NO_KEYCODE, .rotary = 0, .function = 0, .events = NO_EVENT, .hasEvent = 0,
				.time = 0 };
			int fStep = VFO_FREQ_STEP_TABLE[(currentChannelData->VFOflag5 >> 4)];
