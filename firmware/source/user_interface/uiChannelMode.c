...
menuStatus_t uiChannelMode(uiEvent_t *ev, bool isFirstRun) {
	...
	if (isFirstRun) {
		...
		if (ev->events == NO_EVENT) {
			...
			if (ev->hasEvent) {
				if ((trxGetMode() == RADIO_MODE_ANALOG) && (ev->events & KEY_EVENT) && ((ev->keys.key == KEY_LEFT) || (ev->keys.key == KEY_RIGHT))) {
					sqm = ev->time;
				}
	...
	if (CODEPLUG_ZONE_IS_ALLCHANNELS(currentZone)) {
		do {
			do {
				...
				nextChannelIndex = (
						(uiDataGlobal.Scan.direction == 1) ? ((((nextChannelIndex - 1) + 1) % currentZone.NOT_IN_CODEPLUGDATA_highestIndex) + 1) : ((((nextChannelIndex - 1) + currentZone.NOT_IN_CODEPLUGDATA_highestIndex - 1) % currentZone.NOT_IN_CODEPLUGDATA_highestIndex) + 1));
		...
		do {
			...
			nextChannelIndex = (
					(uiDataGlobal.Scan.direction == 1) ? ((nextChannelIndex + 1) % currentZone.NOT_IN_CODEPLUGDATA_numChannelsInZone) : ((nextChannelIndex + currentZone.NOT_IN_CODEPLUGDATA_numChannelsInZone - 1) % currentZone.NOT_IN_CODEPLUGDATA_numChannelsInZone));
...
static void loadChannelData(bool useChannelDataInMemory, bool loadVoicePromptAnnouncement) {
	...
	if ((!uiDataGlobal.VoicePrompts.inhibitInitial || loadVoicePromptAnnouncement) && ((uiDataGlobal.Scan.active == false) || (uiDataGlobal.Scan.active && ((uiDataGlobal.Scan.state = SCAN_SHORT_PAUSED) || (uiDataGlobal.Scan.state = SCAN_PAUSED))))) {
		announceItem(PROMPT_SEQUENCE_CHANNEL_NAME_AND_CONTACT_OR_VFO_FREQ_AND_MODE,
				((nextMenu == UI_TX_SCREEN) || (nextMenu == UI_PRIVATE_CALL) || uiDataGlobal.Scan.active) ? PROMPT_THRESHOLD_NEVER_PLAY_IMMEDIATELY : PROMPT_THRESHOLD_3);
	}
#else
		...
		case QSO_DISPLAY_DEFAULT_SCREEN:
			...
			if (trxTransmissionEnabled) {
				...
				if (uiDataGlobal.displayChannelSettings) {
					...
					uiUtilityDisplayFrequency(DISPLAY_Y_POS_RX_FREQ, false, false, (
							uiDataGlobal.reverseRepeater ? currentChannelData->txFreq : currentChannelData->rxFreq), false, false, 0);
					uiUtilityDisplayFrequency(DISPLAY_Y_POS_TX_FREQ, true, false, (
							uiDataGlobal.reverseRepeater ? currentChannelData->rxFreq : currentChannelData->txFreq), false, false, 0);
				} else {
					if (CODEPLUG_ZONE_IS_ALLCHANNELS(currentZone)) {
						// All Channels virtual zone
			...
			if (!uiDataGlobal.displayChannelSettings) {
				...
				uiUtilityDisplayInformation(nameBuf, DISPLAY_INFO_CHANNEL, (
						trxTransmissionEnabled ? DISPLAY_Y_POS_CHANNEL_SECOND_LINE : -1));
			}
			...
			if (trxGetMode() == RADIO_MODE_DIGITAL) {
				...
				uiUtilityDisplayInformation(nameBuf, DISPLAY_INFO_CONTACT, (
