/**
 * \file menuRSSIScreen.c
 * \brief Handles the display of the menu's RSSI screen
 *
 * \author		Gerad Munsch [KD9QZO] <gmunsch@kd9qzo.com>
 * \date		November 2021
 * \author		Roger Clark [VK3KYY / G4KYF]
 * \date		2019
 *
 * \copyright	Copyright (c) 2021 Gerad Munsch [KD9QZO] \n
...
#if 0
static calibrationRSSIMeter_t rssiCalibration;	/* NOTE: UNUSED */
#endif
...
/*! \brief Holds the RSSI reading in dBm units */
static int dBm = 0;
...
menuStatus_t menuRSSIScreen(uiEvent_t *ev, bool isFirstRun) {
	...
	if (isFirstRun) {
#if 0
		calibrationGetRSSIMeterParams(&rssiCalibration);	/* NOTE: UNUSED */
#endif
...
/**
 * \brief Returns S-Unit (0-9) from RSSI in dBm
 *
 * The \b dBm value of the signal strength is converted into the familiar <em>"S-Unit" strength</em> measurement of the
 * received signal strength.
 * The range for <em>"S-Unit"</em> values are \b S0 (being the weakest signal) to \b S9 (being the strongest signal); in
 * this case, we also have \b S9+10dBm through \b S9+60dBm units.
 *
 * \param[in]	dBm	The \b RSSI of the received radio signal, in \b dBm units.
 *
 * \return A value of \b 0 to \b 15 -- referring to the <em>"S-Unit" strength</em> of the signal, which referenced the
 *         array of values \ref DBM_LEVELS in the file \ref uiGlobals.c
 */
...
static void updateScreen(bool forceRedraw, bool isFirstRun) {
	...
	if (forceRedraw) {
		...
		for (uint8_t i = 0; i < 10; i++) {
			 ...
			 * With DMR signals, the max signal is S9+10, so the entire bar can be the same scale.
			 * With FM signals above S9, the scale is compressed to 2/5
			 */
			...
			if ((i % 2) && (i < 10)) {
				...
				ucPrintAt(((((barX - 2) + xPos) - 2) - 1)/* FONT_2 H offset */, DISPLAY_Y_POS_RSSI_BAR + 15 - 1, buf, FONT_SIZE_2);
#else
				ucPrintAt(((((barX - 2) + xPos) - 2) - 1)/* FONT_2 H offset */, DISPLAY_Y_POS_RSSI_BAR + 15, buf, FONT_SIZE_2);
#endif
			}
		}
	} else {
	...
	/* In FM mode, the max RSSI value from the hardware is over S9+60, so scale to fit in the last 30% of the display */
