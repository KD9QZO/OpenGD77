...
menuStatus_t uiMessageBox(uiEvent_t *ev, bool isFirstRun) {
	...
	if (isFirstRun) {
		...
		if (ev->events == NO_EVENT) {
			...
			if ((uiDataGlobal.MessageBox.type == MESSAGEBOX_TYPE_PIN_CODE) && (uiDataGlobal.FreqEnter.index >= 0) && ((ev->time - curm) > 300)) {
				curm = ev->time;
				forceRedraw = true;
			}
...
static void handleEvent(uiEvent_t *ev) {
	...
	if (KEYCHECK_SHORTUP(ev->keys, KEY_GREEN)) {
		...
		if ((uiDataGlobal.MessageBox.type == MESSAGEBOX_TYPE_PIN_CODE) && (uiDataGlobal.FreqEnter.index < uiDataGlobal.MessageBox.pinLength)) {
			return;
		}
	...
	switch (uiDataGlobal.MessageBox.type) {
		case MESSAGEBOX_TYPE_INFO: {
			if (uiDataGlobal.MessageBox.decoration != MESSAGEBOX_DECORATION_NONE) {
				ucDrawRoundRectWithDropShadow(4, 4, 120, DISPLAY_SIZE_Y - (
						uiDataGlobal.MessageBox.buttons != MESSAGEBOX_BUTTONS_NONE ? FONT_SIZE_3_HEIGHT : 0) - 6, 5, true);
			}
		...
		case MESSAGEBOX_TYPE_PIN_CODE: {
			...
			if (forceRedraw) {
				...
				// Clear input
				ucFillRect(0, (((DISPLAY_SIZE_Y / 8) - 1) * 3) + 2, DISPLAY_SIZE_X, (DISPLAY_SIZE_Y - (((DISPLAY_SIZE_Y / 8) - 1) * 3)) - FONT_SIZE_3_HEIGHT - 2, true);
			}
