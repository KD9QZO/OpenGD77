...
menuDataGlobal_t menuDataGlobal = { .currentItemIndex = 0,  // each menu can re-use this var to hold the position in their display list. To save wasted memory if they each had their own variable
	.startIndex = 0,  // as above
	.endIndex = 0,  // as above
	.lightTimer = -1, .currentMenuList = NULL,

	.controlData = { .stackPosition = 0, .stack = {
		MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY,
		MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY, MENU_EMPTY } },
	...
	NULL,  // splash
		...
		NULL,  // vfo mode
		NULL,  // channel mode
		&menuDataMainMenu, &menuDataContact,
		NULL,  // zone
		NULL,  // Battery
		NULL,  // Firmwareinfo
...
static menuFunctionData_t menuFunctions[] = { { uiSplashScreen, 0 }, { uiPowerOff, 0 }, { uiVFOMode, 0 }, {
	uiChannelMode, 0 }, { menuDisplayMenuList, 0 },  // display Main menu using the menu display system
{ menuDisplayMenuList, 0 },  // display Contact menu using the menu display system
	{ menuZoneList, 0 }, { menuRadioInfos, 0 }, { menuFirmwareInfoScreen, 0 }, { menuNumericalEntry, 0 }, {
		menuTxScreen, 0 }, { menuRSSIScreen, 0 }, { menuLastHeard, 0 }, { menuOptions, 0 }, { menuDisplayOptions, 0 }, {
		menuSoundOptions, 0 }, { menuCredits, 0 }, { menuChannelDetails, 0 }, { menuHotspotMode, 0 }, { uiCPS, 0 }, {
		uiChannelModeQuickMenu, 0 }, { uiVFOModeQuickMenu, 0 }, { menuLockScreen, 0 }, { menuContactList, 0 }, {
		menuContactList, 0 }, { menuContactList, 0 }, { menuContactListSubMenu, 0 }, { menuContactDetails, 0 }, {
		menuContactDetails, 0 }, { menuLanguage, 0 }, { menuPrivateCall, 0 }, { uiMessageBox, 0 } };
...
static void menuSystemPushMenuFirstRun(void) {
	uiEvent_t ev = {
		.buttons = 0, .keys = NO_KEYCODE, .rotary = 0, .function = 0, .events = NO_EVENT, .hasEvent = false,
		.time = fw_millis() };
	menuStatus_t status;
	...
	if (menuDataGlobal.endIndex == INT32_MIN) {
		menuDataGlobal.endIndex = (
				(menuDataGlobal.data[menuDataGlobal.controlData.stack[menuDataGlobal.controlData.stackPosition]] != NULL) ? menuDataGlobal.data[menuDataGlobal.controlData.stack[menuDataGlobal.controlData.stackPosition]]->numItems : 0);
	}

	if (menuDataGlobal.currentMenuList == NULL) {
		menuDataGlobal.currentMenuList = (
				(menuDataGlobal.data[menuDataGlobal.controlData.stackPosition] != NULL) ? (menuItemNewData_t*)menuDataGlobal.data[menuDataGlobal.controlData.stackPosition]->items : NULL);
	}
...
void displayLightTrigger(bool fromKeyEvent) {
	if ((menuSystemGetCurrentMenuNumber() != UI_TX_SCREEN) && (((nonVolatileSettings.backlightMode == BACKLIGHT_MODE_AUTO) || (nonVolatileSettings.backlightMode == BACKLIGHT_MODE_SQUELCH)) || ((nonVolatileSettings.backlightMode == BACKLIGHT_MODE_MANUAL) && displayIsBacklightLit()) || ((nonVolatileSettings.backlightMode == BACKLIGHT_MODE_BUTTONS) && fromKeyEvent))) {
		if ((nonVolatileSettings.backlightMode == BACKLIGHT_MODE_AUTO) || (nonVolatileSettings.backlightMode == BACKLIGHT_MODE_SQUELCH) || (nonVolatileSettings.backlightMode == BACKLIGHT_MODE_BUTTONS)) {
...
const menuItemNewData_t mainMenuItems[] = {
	{ 3, MENU_ZONE_LIST }, { 4, MENU_RSSI_SCREEN }, { 150, MENU_RADIO_INFOS }, { 6, MENU_CONTACTS_MENU }, {
		7, MENU_LAST_HEARD }, { 8, MENU_FIRMWARE_INFO }, { 9, MENU_OPTIONS }, { 10, MENU_DISPLAY }, { 11, MENU_SOUND },
	{ 12, MENU_CHANNEL_DETAILS }, { 13, MENU_LANGUAGE }, { 2, MENU_CREDITS }, };

const menuItemsList_t menuDataMainMenu = { .numItems = 12, .items = mainMenuItems };

static const menuItemNewData_t contactMenuItems[] = { { 15, MENU_CONTACT_LIST }, { 139, MENU_DTMF_CONTACT_LIST }, {
	14, MENU_CONTACT_NEW }, };

const menuItemsList_t menuDataContact = { .numItems = 3, .items = contactMenuItems };
